...
fn find_ocgas(gasses: &[Gas], depth: Pressure, segment_type: SegmentType) -> Gas {
    ...
    for g in gasses {
        if g.use_gas(depth, segment_type) {
            match ret {
                Some(rgas) => if g.f_o2 > rgas.f_o2 { ret = Some(*g); },
                None => ret = Some(*g)
            }
        }
    }
    match ret {
        None => {
            for g in gasses {
                if g.use_gas(depth, segment_type) {
                    match ret {
                        Some(rgas) => if g.f_o2 * depth.to_mbar() < rgas.f_o2 * depth.to_mbar() { ret = Some(*g); },
                        None => ret = Some(*g)
                    }
                }
            }
...
pub fn find_gas(dive: &Dive, gasses: &[Gas], depth: Pressure, segment_type: SegmentType, setpoint: f64) -> Gas {
    match dive.dive_type {
        DiveType::CCR => {
            let mut dil = Gas::new_bottom(0.21, 0.0, 1.4);
            for g in gasses {
                if g.use_diluent { dil = *g; }
            }
...
fn next_gf(gf_slope: f64, dive: &Dive, stop: Pressure) -> f64 {
    if stop.to_mbar() - dive.stop_size.to_mbar() - dive.atm_pressure.to_mbar() < 0.0 { dive.gf_hi}
    else { (gf_slope * (stop.to_mbar() - dive.stop_size.to_mbar() - dive.atm_pressure.to_mbar())) + dive.gf_hi }
}

fn calc_ceiling(comps: &Compartments, atm: Pressure, constants: &TissueConstants, gf: f64) -> Pressure // Depth of current ceiling.
{
    let mut ceiling = 0.0;
...
fn calc_tissue_bottom(tissue_in: f64, time: f64, depth: Pressure, partial_water: f64,
                      partial_pressure: f64, half_time: f64) -> f64 {
...
fn calc_bottom(comps_in: &Compartments, constants: &TissueConstants, partial_water: f64,
               depth: Pressure, time: f64, gas: Gas) -> Compartments {
    let mut comps_out = Compartments::new_empty(COMPARTMENTS);
    if time > 0.0 {
        for i in  0 .. constants.compartments {
            comps_out.nitrogen[i] = calc_tissue_bottom(comps_in.nitrogen[i], time, depth, partial_water,
                                                       gas.f_n2, constants.half_times_n2[i]);
            comps_out.helium[i] = calc_tissue_bottom(comps_in.helium[i], time, depth, partial_water,
                                                     gas.f_he, constants.half_times_he[i]);
        }
    }
    comps_out
}

fn calc_tissue_change(tissue_in: f64, time: f64, depth: Pressure, rate: DepthChange, partial_water: f64,
                      partial_pressure: f64, half_time: f64) -> f64 {
    let bar = depth.to_bar();
...
fn calc_change(comps_in: &Compartments, constants: &TissueConstants, partial_water: f64, gas: Gas,
               rate: DepthChange, from_depth: Pressure, to_depth: Pressure) -> Compartments {
    let mut comps_out = Compartments::new_copy(comps_in);
    let time: f64 = (to_depth.to_mbar() - from_depth.to_mbar()) / rate.to_mbar();
    for i in 0 .. constants.compartments {
        comps_out.nitrogen[i] = calc_tissue_change(comps_in.nitrogen[i], time, from_depth,
                                                   rate, partial_water, gas.f_n2,
                                                   constants.half_times_n2[i]);
        comps_out.helium[i] = calc_tissue_change(comps_in.helium[i], time, from_depth,
                                                 rate, partial_water, gas.f_he,
...
fn next_stop(dive: &Dive, comps: &Compartments, constants: &TissueConstants, gf: f64) -> Pressure // Depth of next stop.
{
    ...
    if stop <= dive.atm_pressure { return dive.atm_pressure; }
    if stop.to_mbar() <= dive.last_stop.to_mbar() { return dive.last_stop; }
...
fn descend(dive: &Dive, constants: &TissueConstants, gasses: &[Gas], rate: DepthChange,
          from_depth: Pressure, to_depth: Pressure, setpoint: f64, comps_in: &Compartments)
          -> (Compartments, Segment) {
    let segment_type = if rate.to_mbar() < 0.0 { SegmentType::UP } else { SegmentType::DOWN };
    let time: f64 = (to_depth.to_mbar() - from_depth.to_mbar()) / rate.to_mbar();
    let gas: Gas = find_gas(dive, gasses, to_depth, segment_type, setpoint);
    let comps_out = calc_change(comps_in, constants, dive.partial_water, gas, rate, from_depth, to_depth);
    ...
    match prev_seg {
        Some(seg) => {
            if seg.segment_type == SegmentType::UP && seg.gas == new_seg.gas {
                time += seg.raw_time;
                otu_cns += seg.otu_cns;
                let seg_out = Segment { time: time.ceil() as u32, raw_time: time, otu_cns, compartments: Compartments::new_copy(&new_seg.compartments), ..new_seg };
                segs.push(seg_out);
                return segs;
            } else {
                segs.push(seg);
...
fn ascend(dive: &Dive, constants: &TissueConstants, gasses: &[Gas], rate: DepthChange,
          from_depth: Pressure, to_depth: Pressure, setpoint: f64, comps_in: &Compartments) -> (Compartments, Segment) {
    descend(dive, constants, gasses, rate, from_depth, to_depth, setpoint, comps_in)
}

fn bottom(dive: &Dive, constants: &TissueConstants, gasses: &[Gas], depth: Pressure,
          time: f64, setpoint: f64, comps_in: &Compartments) -> (Compartments, Segment)  {
    ...
    (comps_out, Segment { segment_type: SegmentType::LEVEL,
        depth,
        raw_time: time,
        time: time.ceil() as u32,
        gas,
...
fn calc_bottom_segment(dive: &Dive, comps_in: &Compartments, constants: &TissueConstants,
                 gas: Gas, depth: Pressure, gf: f64, time_in: f64) -> (Compartments, f64) {
    let mut comps_out = Compartments::new_copy(comps_in);
    let mut time = 0.0;
    let mut done = false;
    let mut first = true;
    while !done {
        ...
        comps_out = calc_bottom(&comps_out, constants, dive.partial_water, depth, segment_time, gas);
        time += 1.0;
...
fn calc_deco_int(dive: &Dive, comps_in: &Compartments, constants: &TissueConstants,
                 gasses: &[Gas], last_depth_in: Pressure, gf: f64, gf_slope: f64) -> (Vec<Segment>, Compartments) {
    ...
    while !main_done {
        ...
        if fs < last_depth {
            let (comps, seg) = ascend(dive, constants, gasses, dive.ascent_rate,
                                            last_depth, fs, dive.deco_setpoint, &comps_out);
            ...
            if seg.raw_time > 1.0 && seg.time as f64 > seg.raw_time {
                ...
                comps_out = calc_bottom(&comps_out, constants, dive.partial_water, fs, time_off, seg.gas);
            }
        }
        if fs <= dive.atm_pressure { return (segments, comps_out); } // At surface, done...
        ngf = next_gf(gf_slope, dive, fs);
        nfs = next_stop(dive, &comps_out, constants, ngf);
        if nfs == fs {
            let gas = find_gas(dive, gasses, fs, SegmentType::UP,
                               dive.deco_setpoint);
            // XXX I want to be a function...
            let time_off = if segments.is_empty() { 0.0 } else {
                if segments[segments.len()-1].segment_type != SegmentType::LEVEL && segments[segments.len()-1].raw_time < 1.0 {
                    segments[segments.len() - 1].raw_time
                } else { 0.0 }
            };
            if time_off > 0.0 { segments.pop(); } // What about otu/cns? XXX TODO
            let (new_comps, time) =
            ...
            last_depth = fs;
            segments.push(Segment { segment_type: SegmentType::LEVEL, depth: fs,
                                            raw_time: time, time: time.ceil() as u32, gas,
                                            ceiling: 0, otu_cns, setpoint: dive.deco_setpoint, compartments: Compartments::new_copy(&comps_out) });
        }
...
fn initial_segments(dive: &Dive, compartments: &Compartments, constants: &TissueConstants,
                    segments_in: &[SegmentIn], gasses: &[Gas]) -> (Vec<Segment>, Compartments, Pressure) {
    ...
    for s in segments_in {
        ...
        let raw_time: f64;
        if last_depth < depth {
            let (comps, seg) = descend(dive, constants, gasses, dive.descent_rate,
                                       last_depth, depth, s.setpoint,&comps_out);
            raw_time = seg.raw_time;
            segments.push(seg);
            comps_out = comps;
        } else {
            let (comps, seg) = ascend(dive, constants, gasses, dive.ascent_rate, last_depth,
                                            depth, s.setpoint, &comps_out);
            raw_time = seg.raw_time;
            segments.push(seg);
            comps_out = comps;
        }
        let (comps, seg) = bottom(dive, constants, gasses, depth, s.time - raw_time, s.setpoint,&comps_out);
        comps_out = comps;
        segments.push(Segment {time: (s.time - raw_time.ceil()) as u32, ..seg });
        last_depth = depth;
    }
    (segments, comps_out, last_depth)
}

fn ascend_to_first_stop(dive: &Dive, compartments: &Compartments, constants: &TissueConstants,
                        gasses: &[Gas], depth: Pressure) -> (Vec<Segment>, Compartments, Pressure) {
    ...
    while !at_first_stop {
        let (comps, seg) = ascend(dive, constants,
                                        gasses, dive.ascent_rate, last_depth, fs,
                                        dive.deco_setpoint, &comps_out);
        comps_out = comps;
        let mut newsegs = merge_ascends(segments.pop(), seg);
        segments.append(&mut newsegs);
        //comps_out = comps;
...
pub fn calc_deco(dive: &Dive, compartments: &Compartments, constants: &TissueConstants,
                 segments_in: &[SegmentIn], gasses: &[Gas]) -> Result<Vec<Segment>, String> {
    if segments_in.len() == 0 { return Err("Must provide segment(s) to calculate deco against.".to_string()); }
    ...
    segments.append(&mut newsegs);
    let gf_slope = (dive.gf_hi - dive.gf_lo) / -((last_depth.to_mbar() - dive.atm_pressure.to_mbar()) as f64);
    let gf = next_gf(gf_slope, dive, last_depth);
    let (mut newsegs, _comps_out) = calc_deco_int(dive, &comps_out, constants, gasses, last_depth, gf, gf_slope);
    segments.append(&mut newsegs);
    Ok(segments)
}

pub fn calc_deco_a(dive: &Dive, compartments: &Compartments, segments: &[SegmentIn], gasses: &[Gas]) -> Result<Vec<Segment>, String> {
    calc_deco(dive, compartments, &CONSTANTS_A, segments, gasses)
}
pub fn calc_deco_b(dive: &Dive, compartments: &Compartments, segments: &[SegmentIn], gasses: &[Gas]) -> Result<Vec<Segment>, String> {
    calc_deco(dive, compartments, &CONSTANTS_B, segments, gasses)
}
pub fn calc_deco_c(dive: &Dive, compartments: &Compartments, segments: &[SegmentIn], gasses: &[Gas]) -> Result<Vec<Segment>, String> {
